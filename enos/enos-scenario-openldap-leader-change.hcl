...
scenario "openldap_leader_change" {
  description = <<-EOF
    The scenario verifies that the Vault OpenLDAP secrets engine plugin works correctly after a leader change.
    ...
    After that, it forces a Vault leader stepdown followed by a leader election and verifies that the plugin still works correctly after the leader change
  ...
  // Wait for our cluster to elect a leader
  step "wait_for_leader_after_step_down" {
    description = global.description.wait_for_cluster_to_have_leader
    module      = module.vault_wait_for_leader
  ...
  step "get_vault_cluster_ips_after_step_down" {
    description = global.description.get_vault_cluster_ip_addresses
    module      = module.vault_get_cluster_ips
    depends_on  = [step.wait_for_leader_after_step_down]
  ...
  step "verify_vault_unsealed_after_step_down" {
    description = global.description.verify_vault_unsealed
    module      = module.vault_wait_for_cluster_unsealed
    depends_on  = [step.wait_for_leader_after_step_down]
  ...
  step "test_static_role_crud_api_after_step_down" {
    description = global.description.static_role_crud_api
    module      = module.static_role_crud_api
    depends_on  = [step.verify_vault_unsealed_after_step_down]
    ...
    variables {
      vault_leader_ip        = step.get_vault_cluster_ips_after_step_down.leader_host.public_ip
      vault_addr             = step.create_vault_cluster.api_addr_localhost
      vault_root_token       = step.create_vault_cluster.root_token
      plugin_mount_path      = var.plugin_mount_path
      ldap_host              = step.create_ldap_server.ldap_ip_address
  ...
  step "test_dynamic_role_crud_api_after_step_down" {
    description = global.description.dynamic_role_crud_api
    module      = module.dynamic_role_crud_api
    depends_on  = [step.verify_vault_unsealed_after_step_down]
  ...
  step "test_library_crud_api_after_step_down" {
    description = global.description.library_crud_api
    module      = module.library_crud_api
    depends_on = [
      step.verify_vault_unsealed_after_step_down,
