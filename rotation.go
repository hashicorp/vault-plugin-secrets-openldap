...
func (b *backend) rotateCredential(ctx context.Context, s logical.Storage) bool {
	...
	if err != nil {
		b.Logger().Error("unable to rotate credentials in periodic function", "rotationID", item.Key, "error", err)
	...
	b.Logger().Info("successfully rotated in periodic function", "rotationID", item.Key)
	return true
}
